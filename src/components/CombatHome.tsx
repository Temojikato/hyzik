...
import {
  combatDescriptionPhrase,
  combatIdentityPhrase,
  reyvateilIdentityPhrase,
  UI_HYMMNOS,
} from '../data/hymmnosInterface';
...
const ResponsiveInfoPanel: React.FC<{ title: React.ReactNode; children: React.ReactNode }> = ({ title, children }) => {
  const [expanded, setExpanded] = useState(false);
  return <>
    <Box display={{ base: 'block', md: 'none' }} border="1px solid" borderColor="whiteAlpha.300" borderRadius="xl" overflow="hidden"><Flex as="button" type="button" w="full" px={3} py={3} align="center" textAlign="left" onClick={() => setExpanded((value) => !value)}><Box fontWeight="bold" flex="1">{title}</Box>{expanded ? <FaChevronUp /> : <FaChevronDown />}</Flex><Collapse in={expanded} animateOpacity><Box px={3} pb={3} pt={2} borderTop="1px solid" borderColor="whiteAlpha.200">{children}</Box></Collapse></Box>
    <Box display={{ base: 'none', md: 'block' }} p={5} borderRadius="xl" border="1px solid" borderColor="whiteAlpha.300"><Box>{title}</Box><Box mt={2}>{children}</Box></Box>
  </>;
};
...
const CombatHome: React.FC<{ reyvateil: Reyvateil; profile: PlayerProfile }> = ({ reyvateil, profile }) => {
  ...
  const activeSongDefinition = activeSong ? songs.find((song) => song.id === activeSong.songId) : undefined;
  const activeSongPhrase = activeSongDefinition && activeSong
    ? { hymmnos: resolveAbilityInvocation(asInvocationAbility(activeSongDefinition)).headword, translation: activeSong.songName }
    : null;
    ...
    <VStack align="stretch" spacing={{ base: 4, md: 6 }} mt={{ base: 3, md: 5 }}>
      <Box display={{ base: 'block', md: 'none' }} p={3} borderRadius="xl" bg="blackAlpha.400" border="1px solid" borderColor={isTurn ? 'green.300' : 'whiteAlpha.300'}>
        <Flex align="center" gap={3}>
          ...
          <Box minW={0} flex="1">
            <HymmnosLabel phrase={reyvateilIdentityPhrase(reyvateil.name, combatProfile.level)} scriptProps={{ fontSize: 'sm', noOfLines: 1 }} translationProps={{ fontSize: '9px' }} />
            <HymmnosLabel mt={1} phrase={combatIdentityPhrase(combat.specialtyTitle, combat.damageType, combat.role)} scriptProps={{ fontSize: 'xl', fontWeight: 'bold', noOfLines: 2 }} translationProps={{ fontSize: 'xs', noOfLines: 2 }} />
          </Box>
        </Flex>
      ...
      <Flex display={{ base: 'none', md: 'flex' }} p={6} borderRadius="2xl" bg="blackAlpha.400" border="1px solid" borderColor={isTurn ? 'green.300' : 'whiteAlpha.300'} gap={5} align="center" wrap="wrap">
        ...
        <Box flex="1" minW="240px">
          <HymmnosLabel phrase={reyvateilIdentityPhrase(reyvateil.name, profile.combatProfile.level)} scriptProps={{ fontSize: 'lg' }} />
          <HymmnosLabel mt={2} phrase={combatIdentityPhrase(combat.specialtyTitle, combat.damageType, combat.role)} scriptProps={{ fontSize: '3xl', fontWeight: 'bold' }} translationProps={{ fontSize: 'sm' }} />
          <HymmnosLabel mt={2} phrase={combatDescriptionPhrase(reyvateil.name, combat.damageType, combat.role)} scriptProps={{ fontSize: 'lg' }} translationProps={{ fontSize: 'sm' }} />
          <Progress mt={4} value={(hp / Math.max(1, maxHp)) * 100} colorScheme={hp / maxHp < .3 ? 'red' : 'green'} borderRadius="full" /><HStack mt={1}><Text fontWeight="bold">{hp} / {maxHp} HP</Text><Button size="xs" variant="ghost" leftIcon={<FaCircleQuestion />} onClick={hpReference.onOpen}>How HP works</Button></HStack>
        </Box>
      ...
      {campaignState.battleActive && <Box p={{ base: 3, md: 4 }} borderRadius="xl" border="1px solid" borderColor={activeSong ? 'pink.300' : 'whiteAlpha.300'} bg={activeSong ? 'pink.900' : 'blackAlpha.300'}>
        <Flex justify="space-between" gap={3} flexWrap="wrap">
          <Box><HymmnosLabel phrase={UI_HYMMNOS.sharedPerformance} scriptProps={{ fontSize: 'md' }} />{activeSongPhrase ? <HymmnosLabel mt={2} phrase={activeSongPhrase} scriptProps={{ fontSize: 'xl', fontWeight: 'bold' }} /> : <Heading size="sm" mt={1}>No Canticle is active</Heading>}{activeSong && <Text mt={1} fontSize={{ base: 'xs', md: 'md' }}>{activeSong.performerName} · {activeSong.stage === 'chanting' ? `chanting until round ${activeSong.activatesAtRound}` : activeSong.endsAfterRound ? `active through round ${activeSong.endsAfterRound}` : 'active until interrupted or combat ends'} · all hearers affected</Text>}</Box>
